...
DaemonArguments.prototype.getHelpText = function DaemonArgumentsGetHelpText() {
    ...
    definition.push('usage: governor [-p|--port <port>] [-n|--node-name <name>] [--priority <priority>] [--log-path <path>] <node>...');
    definition.push('');
    ...
    definition.push(util.format('%s--log-path   %sRotating log path.', tab, tab));
    definition.push(util.format('%s--cluster    %sComma delimited cluster connection string.', tab, tab));
...
DaemonArguments.prototype.validate = function DaemonArgumentsValidate(args) {
    ...
    schema = Joi.object().keys({
        ...
        'log-path': Joi.string().optional().default(null),
        priority: Joi.number().min(0).precision(1).default(1),
        cluster: Joi.array().optional().unique().single().includes(Joi.string()).min(1),
        p: Joi.number().valid(Joi.ref('port')),
        n: Joi.string().valid(Joi.ref('node-name')),
        _: Joi.array().optional().unique().single().includes(Joi.string()).min(1)
    }).xor('cluster', '_');

    if (args.hasOwnProperty('cluster') && typeof args.cluster === 'string') {
