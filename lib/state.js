 ...
 *
 * @param jobinfo
 * @param jobinfo.id {string} - id for locked job, ie: "recasting-23501" or "recasting-23"
 * @param jobinfo.loc - 0 = ok, 1 = locked, 2 = invalid request
 */
State.prototype.inquire = function (jobinfo) {
    if (!jobinfo || !jobinfo.id) {
        return jobStates.INVALID;
    }

    if (this.state[jobinfo.id]) {
        return jobStates.LOCKED;
    }

    if (jobinfo.locking) {
        this.state[jobinfo.id] = 1;
    }
