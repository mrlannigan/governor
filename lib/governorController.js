...
governorController.initialize = function (logger, socket, server, identificationkeys) {
    ...
    socket.onPromise('cluster-place-locks', function (lockData, date, newVersion) {
    ...
    socket.onPromise('cluster-start-job', function (agentname, jobname, date, id) {
        return BPromise.resolve(self.startJob(agentname, jobname, date, id));
    });

    socket.onPromise('cluster-end-job', function (id, date) {
        return BPromise.resolve(self.endJob(id, date));
    });
