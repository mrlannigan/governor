...
agentController.initialize = function (logger, socket, server) {
    ...
    socket.onPromise('job-start', function (agentname, jobname, date) {
        var job;
        date = date || Date.now();
        job = self.startJob(agentname, jobname, date);
        return server.app.state.clusterEmit('cluster-start-job', agentname, jobname, date, job.id)
            .then(function () {
                return job;
            });
    });

    socket.onPromise('job-end', function (id) {
        var date = Date.now();
        self.endJob(id, date);
        return server.app.state.clusterEmit('cluster-end-job', id, date);
    });
};