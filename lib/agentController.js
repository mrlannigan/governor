...
agentController.initialize = function (logger, socket, server) {
    ...
    socket.on('disconnect', function () {
        ...
        socketLogger.info('agent ' + agentname + ' disconnected.  calling clearAgentLocks');

        status = server.app.state.clearAgentLocks(agentname);

        if (status.updated) {
            server.app.state.clusterEmit('cluster-clear-agent-locks', agentname, status.version);
        }
    });
    ...
    socket.onPromise('handle-locks', function (options) {
        ...
        socketLogger.info('handleLocks result', status);
